...
* 当在代码中调用一个抛出InterruptedException异常的方法时,该方法会变为一个阻塞方法，且必须要处理对中断的响应，对库代码来说，有两种基本选择:
    * 传递异常:包括根本不捕获该异常，或者捕获该异常，在执行某种简单的清理工作后再次抛出这个异常
    * 恢复中断:有时不能抛出异常(如当代码时Runnable的一部分时)，此时必须捕获异常，并通过调用当前线程上的interrupt方法恢复中断状态，这样在调用栈更高层的代码将看到引发了一个中断。
> 此外还可以采用更复杂的中断处理方法，但注意不应捕获InterruptedException但不做处理，这样会使调用栈上更高层的代码无法对中断采取处理（此时线程被中断的证据已经丢失），只有对Thread进行扩展时才可以屏蔽中断。
### 同步工具类:
* 阻塞队列不仅可以作为保存对象的容器，还能协调线程之间的控制流.同步工具类可以是任何一个对象,只要它根据其自身状态来协调线程控制流，如阻塞队列，信号量，栅栏，闭锁等，此外还可以根据已有的机制创建自己的同步工具类；
* 所有同步工具类都包含一些特定的结构化属性：封装了一些状态，这些状态决定执行同步工具类的线程是继续执行还是等待，此外提供一些方法对状态进行操作，一次方法用于高效的等待同步工具类进入到预期状态。
* 闭锁:为一种同步工具类，可以延迟线程的进度直到其到达终止状态，作用相当于一扇门(在闭锁到达结束状态之前，这扇门一直关闭，且没有任何线程能通过)。闭锁可以用来确保某些活动直到其他　活动都完成后才执行.
    * CountDownLatch是一种灵活的闭锁实现，可以使一个或多个线程等待一组事件发生。闭锁状态包括一个计数器(初始化为正数), 表示需要等待的事件数量，CountDown方法递减计数器表示一个事件已经发生,await方法等待计数器到达零，表示所需等待的事件全部发生。
